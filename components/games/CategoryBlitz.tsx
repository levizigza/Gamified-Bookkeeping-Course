...
const CATEGORIES: { type: AccountType; label: string; color: string }[] = [
...
export function CategoryBlitz({ week }: { week?: number }) {
  const pool = week
    ? categoryQuestions.filter((q) => q.week <= week)
    : categoryQuestions;
  const ROUND_SIZE = 10;
  const TIME_LIMIT = 90;
  ...
  const answer = (type: AccountType) => {
    const q = questions[idx];
    const correct = type === q.correctCategory;
    const correctLabel = CATEGORIES.find((c) => c.type === q.correctCategory)?.label ?? "";
    if (correct) {
      setScore((s) => s + 1);
  ...
  if (phase === "ready") {
    return (
      <div className="flex flex-col items-center gap-6 py-8 text-center">
        ...
        <h3 className="text-2xl font-bold text-ledger-900">Account Category Blitz</h3>
        <p className="max-w-md text-ledger-600">
          See an account name and sort it into the right category:
          Asset, Liability, Equity, Income, or Expense. {ROUND_SIZE} accounts, {TIME_LIMIT} seconds!
        </p>
